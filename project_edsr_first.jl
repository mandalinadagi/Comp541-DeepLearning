...
ENV["COLUMNS"]=72
using Pkg; for p in ("Plots", "FileIO", "IterTools", "Images", "Colors", "TestImages", "Knet", "ImageIO", "Random", "ImageView"); haskey(Pkg.installed(),p) || Pkg.add(p); end
...
(edsrmmean::EDSRModelMean)(x) = begin
    x[:,:,1]= x[:,:,1].-edsrmmean.mean_hc[1]
    x[:,:,2]= x[:,:,2].-edsrmmean.mean_hc[2]
    x[:,:,3]= x[:,:,3].-edsrmmean.mean_hc[3]
    ypred_meansubt = edsrmmean.edsrm(x)
    ypred_new[:,:,1]= ypred_new[:,:,1] .+ edsrmmean.mean_hc[1]
    ypred_new[:,:,2]= ypred_new[:,:,2] .+ edsrmmean.mean_hc[2]
    ypred_new[:,:,3]= ypred_new[:,:,3] .+ edsrmmean.mean_hc[3]
    ypred_new
...
r_test_130420 = trainresults("", edsrm)
